...
remboursementRouter.post('/', (request, response) => {
    ...
    if(!iddepense || !part|| !utilisateur || !idgroupe){
        response.status(400).json({message: "Bad request: can't create "})
        return
    }
...
remboursementRouter.delete('/:id', (request, response) => {
    ...
    if(!iddepense){
        response.status(400).json({message: "Bad request:can't delete depense"})
        return
    }
